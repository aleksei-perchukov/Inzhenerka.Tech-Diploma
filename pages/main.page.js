...
export class MainPage {
    ...
    async checkLoginName() {
        await test.step('Check user is logged in', async () => {
            ...
            await expect.soft(await this.orderList).toBeVisible({timeout: 20000});
            await expect.soft(await this.userName, `Check username is '${userName}'`).toHaveText(userName, {timeout: 20000});
        })
    }
    ...
    async hideTableTop() {
        await test.step('Click on "Hide TableTop" button', async () => {
            ...
            await expect.soft(await this.showTableTopTitle, "Check 'Show TableTop' title is visible").toBeVisible({timeout: 20000});
        })
    }
    ...
    async selectThickness(thicknessValue) {
        await test.step(`Select '${thicknessValue}' width`, async () => {
            ...
            await expect.soft(await this.thicknessAssert, `Check thickness is ${thicknessValue}`).toHaveText(thicknessValue.toString(), {timeout: 20000});
        })
    }
    ...
    async removePlinth() {
        await test.step(`Click on 'Plinth' button`, async () => {
            ...
            expect.soft(await this.plinthAssert, `Check plinth is removed`).toHaveCount(5, {timeout: 20000});
        })
    }
    ...
    async addIsland() {
        await test.step(`Add 'Island' option`, async () => {    
            ...
            await expect.soft(await this.islandAssert, `Check 'Island' option is added`).toBeVisible({timeout: 20000});
        })
    }
    ...
    async addWaterDrainage() {
        await test.step(`Add 'Water drainage' option`, async () => {
            ...
            await expect.soft(await this.waterAssert, `Check 'Water drainage' option is added`).toHaveText(text, {timeout: 20000});
        })
    }
    ...
    async selectColor(colorValue) {
        await test.step(`Select '${colorValue}' color`, async () => {
            ...
            await expect.soft(await this.colorAssert, `Check color is ${colorValue}`).toContainText(colorValue, {timeout: 20000});
        })
    }
    ...
    async clickCalculateButton() {
        await test.step('Click on "Calculate" button', async () => {
            ...
            await expect(this.reportButton, `Check '${text}' button is clickable`).toHaveText('Расчет', {timeout: 20000});
        })
    }

    async clickReportButton() {
        await test.step(`Click on 'Report' button`, async () => {
            await this.reportButton.click({timeout: 20000});
        })
    }
}
